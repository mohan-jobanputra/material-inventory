...
Class ProductManager
{
    ...
    function updateProduct($product_id, $product, $color, $selling_price)
    {
        $link = mysqli_connect(DB_HOST, DB_USERNAME, DB_PASSWORD, DB_NAME);

        if (isset($product_id, $product, $color, $selling_price))  {
            //The insertion
            $statement = $link->prepare("UPDATE products SET product=?, selling_price=? WHERE product_id=?");
            $statement->bind_param("ssiii", $product, $color, $selling_price, $product_id);
            $statement->execute();
        } else {
    ...
    function autocomplete()
    {
        ...
        $product_query = "SELECT product FROM products" ;
